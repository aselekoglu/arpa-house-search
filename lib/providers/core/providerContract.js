...
export const validateProviderAdapter = (adapter) => {
  ...
  if (!Array.isArray(adapter.domains) || adapter.domains.length === 0) {
    throw new ProviderContractError('domains', 'Provider domains must be a non-empty array');
  }
  const domains = [...new Set(adapter.domains.map(normalizeProviderDomain))];
