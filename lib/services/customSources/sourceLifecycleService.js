...
export const createCustomSourceLifecycleService = ({
  ...
  const activationFor = (source) => {
    let currentRecipeHash = null;
    ...
    const exactPassingTest =
      recipeValid && source.lastTestStatus === 'pass' && source.lastTestRecipeHash === currentRecipeHash;

    return {
      recipeValid,
      canEnable: exactPassingTest,
      needsRetest: source.lastTestStatus != null && !exactPassingTest,
    };
  };

  const present = (source) => ({
    ...source,
    activation: activationFor(source),
  });
  ...
  const service = {
    listSources({ userId }) {
      return storage.listByUser(userId).map(present);
    },

    getSource({ userId, sourceId }) {
      return present(ownedSource(userId, sourceId));
    },
    ...
    saveDraft({ userId, sourceId = null, name, recipe }) {
      ...
      return present(storage.upsert(row));
    },
    ...
    setEnabled({ userId, sourceId, enabled }) {
      ...
      if (shouldEnable) {
        const activation = activationFor(source);
        if (!activation.recipeValid) {
          throw new CustomSourceActivationError('Current Custom Source recipe is not valid');
        }
        if (!activation.canEnable) {
          throw new CustomSourceActivationError();
        }
      }

      return present(storage.setEnabled({ id: source.id, enabled: shouldEnable, updatedAt: now() }));
    },
