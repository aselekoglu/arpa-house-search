...
export const createSearchProfileLifecycleService = ({
  ...
  const assertCustomSourceOwnership = (ownerId, enabledSources) => {
    for (const source of enabledSources) {
      if (source.kind !== 'custom-source') continue;
      const customSource = customSourceStorage.getById(source.id);
      if (!customSource || customSource.userId !== ownerId) {
        throw new SearchProfileOwnershipError('Custom Source is not available to the current user');
      }
    }
  };
  ...
  return Object.freeze({
    ...
    saveProfile({
      ...
      assertCustomSourceOwnership(ownerId, profile.enabledSources);
